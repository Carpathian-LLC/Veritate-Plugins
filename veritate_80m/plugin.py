...
# ------------------------------------------------------------------------------------
# Notes:
# - veritate_80m trainer plugin. THE DEFAULT base model trainer at 80M.
#   pure-vanilla Veritate, tuned for byte-level TinyStories coherence and fast
#   iteration. engine-deployable as a v9 INT8 binary.
# - two-phase recipe (recommended):
#     phase A: qat_enabled=false, total_steps 10000. learns real conditional
#              representations at peak_lr=5e-4 with cosine decay (smaller models
#              tolerate higher LR than 200m).
#     phase B: qat_enabled=true with resume=<phase_A_name>. fine-tunes for
#              ~2500-3500 more steps at peak_lr=1e-4 with warmup_steps=200.
...
# plugins/veritate_80m/plugin.py
# ------------------------------------------------------------------------------------
# Imports
...
def main():
    ...
    if qat_source is not None:
        ...
        if getattr(args, "name", "").strip():
            base = compose_name(args.name, args.size)
